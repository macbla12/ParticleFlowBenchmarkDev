...
void Plotting()
{
    ...
    TFile* ElectronFile = TFile::Open("Plots/Electron.root", "READ");
    TFile* PionFile = TFile::Open("Plots/Pion.root", "READ");
    if (!ElectronFile || ElectronFile->IsZombie()) {
        std::cerr << "ERROR: Could not open Plots/Electron.root\n";
        return;
    }
    if (!PionFile || PionFile->IsZombie()) {
        std::cerr << "ERROR: Could not open Plots/Pion.root\n";
        return;
    }

    DrawSpeciesPlots("Electron", ElectronFile, kBlue);
    DrawSpeciesPlots("Pion", PionFile, kRed);
