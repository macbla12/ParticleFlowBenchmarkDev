...
// Check with one cluster if not check another calorimeter
// All calorimeters
...
const Options ElectronNEndCapOptions = {
  "/run/media/epic/Data/Benchmarking/Single/Electron/Electron.NEndCap.root",
...
const Options ElectronBarrelOptions = {
  "/run/media/epic/Data/Benchmarking/Single/Electron/Electron.Barrel.root",
...
const Options PionLFOptions = {
  "/run/media/epic/Data/Benchmarking/Single/Pion/Pion.PEndCap.root",
...
const Options PionPEndCapOptions = { 
  "/run/media/epic/Data/Benchmarking/Single/Pion/Pion.Insert.root",
  "Plots/PionPEndCap.root",
  "HcalEndcapPInsertClusters",
  "CalorimeterTrackProjections",
   115, //< see https://github.com/eic/epic/blob/main/compact/definitions.xml
   false,
   10,
   true
};
...
const Options PionNEndCapOptions = { 
  "/run/media/epic/Data/Benchmarking/Single/Pion/Pion.NEndCap.root",
...
const Options PionBarrelOptions = {
  "/run/media/epic/Data/Benchmarking/Single/Pion/Pion.Barrel.root",
...
// macro body -----------------------------------------------------------------
void BenchmarkTrackCluster(const Options& opt = PionPEndCapOptions,uint64_t s_use=1) {
    ...
    TH1D* MCEta= new TH1D("MCEta","MCEta",300,-3.8,3.8);
    ...
    TH1D* MCEtaHited= new TH1D("MCEtaHited","MCEtaHited",300,-3.8,3.8);
    ...
    TH1D* EClusteroverETrack= new TH1D("EClusteroverETrack","E_{Cl}/E_{Tr};Ratio E_{Cl}/E_{Tr};Events",100,0,2);
    TH1D* EMCoverECluster= new TH1D("EMCoverECluster","E_{MC}/E_{Cl};Ratio E_{MC}/E_{Cl};Events",100,0,2);
    TH1D* EMCoverETrack= new TH1D("EMCoverETrack","E_{MC}/E_{Tr};Ratio E_{MC}/E_{Tr};Events",100,0,2);
    

    TH2D* EnergyRatiovsTrackPt= new TH2D("EnergyRatiovsTrackPt","E_{Cl}/E_{Tr} vs TrackPt;pT [GeV];Ratio E_{Cl}/E_{Tr}",45,0,9,100,0,2);
    TH2D* EnergyRatiovsTrackEta= new TH2D("EnergyRatiovsTrackEta","EnergyRatiovsTrackEta;#eta;Ratio E_{Cl}/E_{Tr}",50,-3.5,3.5,100,0,2);
    ...
    // ECluster-f*ETrack
    TH1D* EClustMinusFsubEtrk = new TH1D("EClustMinusFsubEtrk",
        "E_{clust} - f_{sub}E_{trk};E_{clust} - f_{sub}E_{trk} [GeV];Events",200,-10,10);
    ...
    cout<<"    Starting frame loop: "<<reader.getEntries(podio::Category::Event)<<" frames to process."<<endl;
    ...
    if (!mcEtaHit.empty()) {
        double etaMin = *min_element(mcEtaHit.begin(), mcEtaHit.end());
        double etaMax = *max_element(mcEtaHit.begin(), mcEtaHit.end());

        int nInRange = count_if(mcEtaAll.begin(), mcEtaAll.end(),
                                    [etaMin, etaMax](double eta) {
        ...
        cout<<"  Average number of cluster matched "<<nClustMatched/MCEtaHited->GetEntries()<<" to particle"<<endl;
    }

    cout<<"    Finished frame loop: "<<nClustMatched<<"/"<<nClustTotal<<" clusters matched."<<endl;

    // compute f_sub and thresh from the E/p distribution
    
 
    // fill the f_sub- and thresh-dependent histograms from the buffered data
    // --- find best (smallest dR) match per event ---
